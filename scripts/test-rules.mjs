...
try {
  ...
  // --- Die Mini-Games ----------------------------------------------------------
  // config/miniGames nennt die Spiele, die es unter kids.alae.app/mini-games
  // einzeln gibt. Dieselbe Regel wie für die freien Spiele – lesen alle,
  // schreiben nur der Admin.
  await darf("Gast liest die Mini-Games", () => gast().doc("config/miniGames").get());
  await darf("Admin macht ein Spiel zum Mini-Game", () => admin().doc("config/miniGames").set({ spiele: ["towerStack"], updatedAtMs: 1 }));
  await darfNicht("Gast macht ein Spiel zum Mini-Game", () => gast().doc("config/miniGames").set({ spiele: ["towerStack"] }));
  await darfNicht("Kind macht ein Spiel zum Mini-Game", () => anna().doc("config/miniGames").set({ spiele: ["towerStack"] }));

  // Und die Bestenliste dazu: miniScores. Sie ist der einzige Ort in dieser
  // Datenbank, an den jemand ohne Konto schreiben darf, ohne dass ihm vorher
  // eine Kennung zugeteilt wurde – deshalb steht hier die längste Liste.
  //
  // Der Name des Dokuments ist spiel + "_" + spieler. Das ist keine
  // Formsache: Er sorgt dafür, dass jeder Spieler je Spiel genau eine Zeile
  // hat. Wer sich eine zweite anlegen könnte, stünde zweimal in der Liste.
  const MINI = "miniScores/towerStack_mini_abcdefghijkl";
  const miniEintrag = (aenderung = {}) => ({
    game: "towerStack",
  ...
  await darf("Gast liest die Bestenliste der Mini-Games", () => gast().collection("miniScores").get());
  await darfNicht("Gast legt einen Eintrag unter fremdem Namen an", () => gast().doc("miniScores/towerStack_mini_xxxxxxxxxxxx").set(miniEintrag()));
  ...
  await darfNicht("Gast legt einen Eintrag mit zweitem Versuch an", () => gast().doc(MINI).set(miniEintrag({ versuche: 2 })));
  await darf("Gast trägt sich in die Bestenliste ein", () => gast().doc(MINI).set(miniEintrag()));

  // Ab hier gibt es den Eintrag. Was daran noch geändert werden darf, ist
  // genau das, was eine weitere Runde ändert.
  ...
  await darfNicht("Gast schiebt seinen Eintrag in ein anderes Spiel", () => gast().doc(MINI).set({ game: "fishPond", name: "Alain", punkte: 42, versuche: 5, updatedAtMs: 5 }, { merge: true }));
  await darfNicht("Gast übernimmt den Eintrag mit einer anderen Kennung", () => gast().doc(MINI).set({ spieler: "mini_zzzzzzzzzzzz", name: "Alain", punkte: 42, versuche: 5, updatedAtMs: 5 }, { merge: true }));

  // Löschen darf nur der Admin – für den Tag, an dem jemand einen Namen
  // einträgt, den ein Kind nicht lesen soll. Zuletzt geprüft: Danach ist der
  // Eintrag weg.
  await darfNicht("Gast löscht einen Eintrag", () => gast().doc(MINI).delete());
  await darfNicht("Kind löscht einen Eintrag", () => anna().doc(MINI).delete());
  await darf("Admin löscht einen Eintrag", () => admin().doc(MINI).delete());
