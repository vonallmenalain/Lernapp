...
const DIFFICULTIES = {
  easy: { label: "Wiese", code: 1, icon: "🌱" },
  medium: { label: "Wald", code: 2, icon: "🌲" },
  hard: { label: "Meer", code: 3, icon: "🌊" },
  extreme: { label: "Weltall", code: 4, icon: "🚀" },
  // Der Garten liegt vor der Wiese: ein paar ganz kleine Rätsel für die
  // Kleinsten. Nur Battleships und Tiergehege haben ihn, und zu sehen bekommt
  // ihn nur, wer auf der Stufe "leicht" spielt (journey-plan.js, stufe()).
  // Er steht nicht in DIFFICULTY_KEYS: die vier Welten bleiben die vier Welten.
  starter: { label: "Garten", code: 0, icon: "🌷" },
};
const DIFFICULTY_KEYS = ["easy", "medium", "hard", "extreme"];
const STARTER_KEY = "starter";
const STARTER_GAMES = new Set(["bimaru", "shikaku"]);
const LEVELS_PER_DIFFICULTY = 10;
const LEVELS_PER_STARTER = 6;

// Die Schwierigkeitsstufe des Kindes (leicht, mittel, schwer): journey-plan.js
// hält sie im Kasten der Reise. Ohne die Datei (das Prüfskript) gilt die Mitte.
function currentStufe() {
  const stufe = window.LernappReise?.stufe?.();
  return stufe === "leicht" || stufe === "schwer" ? stufe : "mittel";
}
// Welche Welten ein Spiel gerade zeigt: den Garten nur auf "leicht".
function worldsFor(game) {
  return STARTER_GAMES.has(game) && currentStufe() === "leicht" ? [STARTER_KEY, ...DIFFICULTY_KEYS] : DIFFICULTY_KEYS;
}
...
const BIMARU_LEVEL_COUNTS = { starter: 0, easy: 0, medium: 0, hard: 0, extreme: 0 };
const BIMARU_LEVELS = [
  // Der Garten: vier mal vier, ein Zweier und zwei Einer – und zwei der vier
  // Schiffsteile stehen von Anfang an da (bimaruGivens).
  ["starter", sea(["1100","0000","0010","1000"], {"1":2,"2":1})],
  ["starter", sea(["1000","1000","0001","0100"], {"1":2,"2":1})],
  ["starter", sea(["0000","0110","0000","1001"], {"1":2,"2":1})],
  ["starter", sea(["0010","0000","1100","0001"], {"1":2,"2":1})],
  ["starter", sea(["0100","0100","0000","1001"], {"1":2,"2":1})],
  ["starter", sea(["0001","0000","0011","1000"], {"1":2,"2":1})],
...
const SHIKAKU_LEVEL_TITLES = {
  starter: ["Erstes Gehege", "Zwei Zäune", "Kleiner Hof", "Drei Ställe", "Hühnerhof", "Ponykoppel"],
...
const SHIKAKU_LEVELS = [STARTER_KEY, ...DIFFICULTY_KEYS].flatMap((difficulty) =>
...
const READING_IMAGE_MAP = {
  ...
  cat_milk: "🐈🥛", dog_bone: "🐕🦴", rain: "🌧️", tree_apples: "🌳🍎", child_reading: "🧒📖", child_sleeping: "🧒💤", frog: "🐸", duck: "🦆",
};
...
// Welche Aufgabenarten eine Welt stellt – je Schwierigkeitsstufe des Kindes
// (journey-plan.js). "leicht" ist der Wortdetektiv von früher: Buchstaben,
// dann Wörter, Sätze erst im Weltall. "mittel" fängt schon auf der Wiese mit
// Wörtern an und liest im Meer erste Sätze; "schwer" liest von der Wiese an
// Wörter und Silben, ab dem Wald Sätze, und ergänzt im Weltall Sätze um das
// passende Wort.
const READING_TASK_TYPES = {
  ...
  mittel: {
    easy: ["missingLetter", "chooseWord"],
    medium: ["chooseWord", "missingSyllable"],
    hard: ["missingSyllable", "chooseWord", "sentenceMatch"],
    extreme: ["sentenceMatch", "sentenceMissingWord", "sentenceComplete"],
  },
  schwer: {
    easy: ["chooseWord", "missingSyllable"],
    medium: ["missingSyllable", "sentenceMatch"],
    hard: ["sentenceMatch", "sentenceMissingWord"],
    extreme: ["sentenceMissingWord", "sentenceComplete"],
  },
};
// Drei Arten von Sätzen:
//   sentenceMatch        Welcher Satz passt zum Bild? subject sagt, wovon der
//                        Satz handelt – ein zweiter Satz über dasselbe Tier
//                        taugt nicht als falsche Antwort
//   sentenceMissingWord  Der ___ frisst. Das Bild zeigt das fehlende Hauptwort
//                        (correctWordId, ein Wort aus READING_WORD_ITEMS)
//   sentenceComplete     Der Hund ___ im Garten. Das fehlende Wort ist ein
//                        Tun- oder Wiewort; answer und distractors stehen dabei
const READING_SENTENCE_ITEMS = [
  { id: "hund-rennt", sentence: "Der Hund rennt.", imageKey: "dog_running", subject: "hund", correctWordIds: ["hund"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "katze-schlaeft", sentence: "Die Katze schläft.", imageKey: "cat_sleeping", subject: "katze", correctWordIds: ["katze"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "auto-faehrt", sentence: "Das Auto fährt.", imageKey: "car", subject: "auto", correctWordIds: ["auto"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "fisch-schwimmt", sentence: "Der Fisch schwimmt.", imageKey: "fish", subject: "fisch", correctWordIds: ["fisch"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "vogel-fliegt", sentence: "Der Vogel fliegt.", imageKey: "bird_flying", subject: "vogel", correctWordIds: ["vogel"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "sonne-scheint", sentence: "Die Sonne scheint.", imageKey: "sun", subject: "sonne", correctWordIds: ["sonne"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "ball-rollt", sentence: "Der Ball rollt.", imageKey: "ball", subject: "ball", correctWordIds: ["ball"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "kind-isst", sentence: "Das Kind isst.", imageKey: "child_eating", subject: "kind-isst", correctWordIds: [], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "buch-rot", sentence: "Das Buch ist rot.", imageKey: "red_book", subject: "buch", correctWordIds: ["buch"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "mond-hell", sentence: "Der Mond ist hell.", imageKey: "moon", subject: "mond", correctWordIds: ["mond"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "katze-milch", sentence: "Die Katze trinkt Milch.", imageKey: "cat_milk", subject: "katze", correctWordIds: ["katze"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "hund-knochen", sentence: "Der Hund hat einen Knochen.", imageKey: "dog_bone", subject: "hund", correctWordIds: ["hund"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "es-regnet", sentence: "Es regnet.", imageKey: "rain", subject: "regen", correctWordIds: [], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "apfel-baum", sentence: "Der Apfel hängt am Baum.", imageKey: "tree_apples", subject: "apfel", correctWordIds: ["apfel", "baum"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "kind-liest", sentence: "Das Kind liest ein Buch.", imageKey: "child_reading", subject: "kind-liest", correctWordIds: ["buch"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "kind-schlaeft", sentence: "Das Kind schläft.", imageKey: "child_sleeping", subject: "kind-schlaeft", correctWordIds: [], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "blume-gelb", sentence: "Die Blume ist gelb.", imageKey: "flower", subject: "blume", correctWordIds: ["blume"], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "frosch-huepft", sentence: "Der Frosch hüpft.", imageKey: "frog", subject: "frosch", correctWordIds: [], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  { id: "ente-schwimmt", sentence: "Die Ente schwimmt.", imageKey: "duck", subject: "ente", correctWordIds: [], difficulty: "extreme", taskTypes: ["sentenceMatch"] },
  ...
  { id: "katze-schlaeft-luecke", sentence: "Die Katze schläft.", prompt: "Die ___ schläft.", imageKey: "cat_sleeping", correctWordId: "katze", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "vogel-fliegt-hoch", sentence: "Der Vogel fliegt hoch.", prompt: "Der ___ fliegt hoch.", imageKey: "bird_flying", correctWordId: "vogel", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "fisch-im-wasser", sentence: "Der Fisch schwimmt im Wasser.", prompt: "Der ___ schwimmt im Wasser.", imageKey: "fish", correctWordId: "fisch", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "sonne-scheint-hell", sentence: "Die Sonne scheint hell.", prompt: "Die ___ scheint hell.", imageKey: "sun", correctWordId: "sonne", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "mond-ist-hell", sentence: "Der Mond ist hell.", prompt: "Der ___ ist hell.", imageKey: "moon", correctWordId: "mond", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "haus-ist-gross", sentence: "Das Haus ist gross.", prompt: "Das ___ ist gross.", imageKey: "house", correctWordId: "haus", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "baum-ist-gruen", sentence: "Der Baum ist grün.", prompt: "Der ___ ist grün.", imageKey: "tree", correctWordId: "baum", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "blume-ist-gelb-luecke", sentence: "Die Blume ist gelb.", prompt: "Die ___ ist gelb.", imageKey: "flower", correctWordId: "blume", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "schuh-passt", sentence: "Der Schuh passt gut.", prompt: "Der ___ passt gut.", imageKey: "shoe", correctWordId: "schuh", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "banane-ist-gelb-luecke", sentence: "Die Banane ist gelb.", prompt: "Die ___ ist gelb.", imageKey: "banana", correctWordId: "banane", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "rakete-zum-mond", sentence: "Die Rakete fliegt zum Mond.", prompt: "Die ___ fliegt zum Mond.", imageKey: "rocket", correctWordId: "rakete", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "krokodil-zaehne", sentence: "Das Krokodil hat viele Zähne.", prompt: "Das ___ hat viele Zähne.", imageKey: "crocodile", correctWordId: "krokodil", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "elefant-ist-gross-luecke", sentence: "Der Elefant ist gross.", prompt: "Der ___ ist gross.", imageKey: "elephant", correctWordId: "elefant", difficulty: "extreme", taskTypes: ["sentenceMissingWord"] },
  { id: "hund-rennt-garten", sentence: "Der Hund rennt im Garten.", prompt: "Der Hund ___ im Garten.", imageKey: "dog_running", answer: "rennt", distractors: ["schläft", "schwimmt", "fliegt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "katze-schlaeft-sofa", sentence: "Die Katze schläft auf dem Sofa.", prompt: "Die Katze ___ auf dem Sofa.", imageKey: "cat_sleeping", answer: "schläft", distractors: ["rennt", "singt", "fliegt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "vogel-fliegt-himmel", sentence: "Der Vogel fliegt am Himmel.", prompt: "Der Vogel ___ am Himmel.", imageKey: "bird_flying", answer: "fliegt", distractors: ["schwimmt", "schläft", "klettert"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "fisch-schwimmt-see", sentence: "Der Fisch schwimmt im See.", prompt: "Der Fisch ___ im See.", imageKey: "fish", answer: "schwimmt", distractors: ["fliegt", "hüpft", "liest"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "sonne-scheint-himmel", sentence: "Die Sonne scheint am Himmel.", prompt: "Die Sonne ___ am Himmel.", imageKey: "sun", answer: "scheint", distractors: ["schläft", "regnet", "fährt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "kind-isst-apfel-satz", sentence: "Das Kind isst einen Apfel.", prompt: "Das Kind ___ einen Apfel.", imageKey: "child_eating", answer: "isst", distractors: ["liest", "malt", "fährt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "kind-liest-buch-satz", sentence: "Das Kind liest ein Buch.", prompt: "Das Kind ___ ein Buch.", imageKey: "child_reading", answer: "liest", distractors: ["isst", "trinkt", "wirft"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "auto-faehrt-strasse", sentence: "Das Auto fährt auf der Strasse.", prompt: "Das Auto ___ auf der Strasse.", imageKey: "car", answer: "fährt", distractors: ["schwimmt", "fliegt", "schläft"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "ball-rollt-rasen", sentence: "Der Ball rollt über den Rasen.", prompt: "Der Ball ___ über den Rasen.", imageKey: "ball", answer: "rollt", distractors: ["schläft", "liest", "singt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "frosch-huepft-teich", sentence: "Der Frosch hüpft in den Teich.", prompt: "Der Frosch ___ in den Teich.", imageKey: "frog", answer: "hüpft", distractors: ["liest", "malt", "singt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "heute-regnet-es", sentence: "Heute regnet es.", prompt: "Heute ___ es.", imageKey: "rain", answer: "regnet", distractors: ["schneit", "singt", "schläft"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "blume-ist-gelb-satz", sentence: "Die Blume ist gelb.", prompt: "Die Blume ist ___.", imageKey: "flower", answer: "gelb", distractors: ["blau", "nass", "laut"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "apfel-ist-rot", sentence: "Der Apfel ist rot.", prompt: "Der Apfel ist ___.", imageKey: "apple", answer: "rot", distractors: ["blau", "gelb", "grün"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "banane-ist-gelb-satz", sentence: "Die Banane ist gelb.", prompt: "Die Banane ist ___.", imageKey: "banana", answer: "gelb", distractors: ["rot", "blau", "grün"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "elefant-ist-gross-satz", sentence: "Der Elefant ist gross.", prompt: "Der Elefant ist ___.", imageKey: "elephant", answer: "gross", distractors: ["klein", "leise", "rot"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "mond-scheint-nacht", sentence: "Der Mond scheint in der Nacht.", prompt: "Der Mond ___ in der Nacht.", imageKey: "moon", answer: "scheint", distractors: ["schwimmt", "hüpft", "fährt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
  { id: "ente-schwimmt-teich", sentence: "Die Ente schwimmt auf dem Teich.", prompt: "Die Ente ___ auf dem Teich.", imageKey: "duck", answer: "schwimmt", distractors: ["liest", "malt", "fährt"], difficulty: "extreme", taskTypes: ["sentenceComplete"] },
];
...
// Wie viele Antworten zur Wahl stehen: drei für die Kleinen, vier sonst, auf
// "schwer" im Weltall fünf.
function readingOptionCount(difficulty, stufe = "mittel") {
  if (stufe === "schwer") return difficulty === "extreme" ? 5 : 4;
...
function generateMissingLetterTask(difficulty, stufe = "mittel") {
  ...
  return {
    ...
    options: ensureUniqueTextOptions(correctAnswer, candidates, readingOptionCount(difficulty, stufe)),
    correctAnswer,
    wordId: item.id,
    fullText: item.word,
    missingIndex,
...
function chooseWordDistractors(item, difficulty, optionCount, stufe = "mittel") {
  const pool = wordsForReadingDifficulty(difficulty, stufe)
...
function generateChooseWordTask(difficulty, stufe = "mittel") {
  const pool = wordsForReadingDifficulty(difficulty, stufe).filter((item) => item.allowedTaskTypes.includes("chooseWord"));
  const item = pickRandom(pool);
  const optionCount = readingOptionCount(difficulty, stufe);
  return {
    ...
    displayText: "",
    options: ensureUniqueTextOptions(item.word, chooseWordDistractors(item, difficulty, optionCount, stufe), optionCount),
    correctAnswer: item.word,
    wordId: item.id,
    fullText: item.word,
  };
}
function generateMissingSyllableTask(difficulty, stufe = "mittel") {
  ...
  return {
    ...
    options: ensureUniqueTextOptions(correctAnswer, candidates, Math.max(4, readingOptionCount(difficulty, stufe))),
    correctAnswer,
    wordId: item.id,
    fullText: item.word,
    missingIndex,
...
function generateSentenceMatchTask(difficulty, stufe = "mittel") {
  const item = pickRandom(READING_SENTENCE_ITEMS.filter((sentence) => sentence.taskTypes.includes("sentenceMatch")));
  const candidates = shuffleOptions(READING_SENTENCE_ITEMS
    .filter((sentence) => sentence.id !== item.id && sentence.taskTypes.includes("sentenceMatch") && sentence.subject !== item.subject)
  ...
  return {
    ...
    displayText: "",
    options: ensureUniqueTextOptions(item.sentence, candidates, Math.max(4, readingOptionCount(difficulty, stufe))),
    correctAnswer: item.sentence,
    sentenceId: item.id,
    fullText: item.sentence,
  };
}
function generateSentenceMissingWordTask(difficulty, stufe = "mittel") {
  ...
  // Zuerst die Wörter mit demselben Artikel: "Die ___ ist gelb" mit Katze,
  // Blume und Banane zur Wahl verrät nichts über den Artikel.
  const candidates = shuffleOptions(READING_WORD_ITEMS.filter((candidate) => candidate.id !== word.id))
    .sort((a, b) => Number(b.article === word.article) - Number(a.article === word.article))
...
// Der Hund ___ im Garten: das Tun- oder Wiewort fehlt. Die falschen Wörter
// stehen beim Satz – zufällige Wörter aus der Liste passten grammatisch nicht.
function generateSentenceCompleteTask(difficulty, stufe = "mittel") {
  const item = pickRandom(READING_SENTENCE_ITEMS.filter((sentence) => sentence.taskTypes.includes("sentenceComplete")));
  return {
    id: `reading-${difficulty}-complete-${Date.now()}-${randomInt(1000, 9999)}`,
    puzzleType: "readingPuzzle",
    difficulty,
    taskType: "sentenceComplete",
    imageKey: item.imageKey,
    prompt: "Welches Wort passt in den Satz?",
    displayText: item.prompt,
    options: ensureUniqueTextOptions(item.answer, shuffleOptions(item.distractors), 4),
    correctAnswer: item.answer,
    sentenceId: item.id,
    fullText: item.sentence,
  };
}
const READING_GENERATORS = {
  missingLetter: generateMissingLetterTask,
  chooseWord: generateChooseWordTask,
  missingSyllable: generateMissingSyllableTask,
  sentenceMatch: generateSentenceMatchTask,
  sentenceMissingWord: generateSentenceMissingWordTask,
  sentenceComplete: generateSentenceCompleteTask,
};
function readingTaskTypesFor(difficulty, stufe) {
  const table = READING_TASK_TYPES[stufe] || READING_TASK_TYPES.mittel;
  return table[difficulty] || table.easy;
}
// Eine Aufgabe für Welt und Stufe. Eine ausdrücklich gewünschte Art (das
// Prüfskript fragt jede einzeln ab) kommt auch dann, wenn die Welt sie auf
// dieser Stufe nicht von sich aus stellte.
function generateReadingTask(difficulty, taskType = null, stufe = currentStufe()) {
  const safeDifficulty = READING_TASK_TYPES.leicht[difficulty] ? difficulty : "easy";
  const safeStufe = READING_TASK_TYPES[stufe] ? stufe : "mittel";
  const type = READING_GENERATORS[taskType] ? taskType : pickRandom(readingTaskTypesFor(safeDifficulty, safeStufe));
  const task = READING_GENERATORS[type](safeDifficulty, safeStufe);
  return validateReadingTask(task) ? task : generateReadingTask(safeDifficulty, taskType, safeStufe);
}
...
function practiceTaskKey(task) {
  if (task.puzzleType === "letterPuzzle") return letterTaskKey(task);
  return readingTaskKey(task);
}
...
// Welche Fragen die Buchstaben-Jagd stellt – je Welt und Schwierigkeitsstufe
// des Kindes (journey-plan.js):
//   start      Womit beginnt Banane? (die Jagd von früher)
//   end        Womit endet Banane?
//   gap        Ban_ne – welcher Buchstabe fehlt?
//   wordStart  Welches Wort beginnt mit B? (zur Wahl stehen Wörter)
//   wordEnd    Welches Wort endet mit E?
// "leicht" bleibt bei den Anfangsbuchstaben; "mittel" nimmt ab dem Wald das
// Ende dazu und ab dem Meer die Wörter zum Buchstaben; "schwer" fragt schon
// auf der Wiese nach dem Ende und ab dem Wald nach Buchstaben mitten im Wort.
const LETTER_MODES = {
  leicht: { easy: ["start"], medium: ["start"], hard: ["start"], extreme: ["start", "end"] },
  mittel: { easy: ["start"], medium: ["start", "end"], hard: ["start", "end", "wordStart"], extreme: ["end", "wordStart", "gap"] },
  schwer: { easy: ["start", "end"], medium: ["end", "gap"], hard: ["gap", "wordStart"], extreme: ["gap", "wordStart", "wordEnd"] },
};
function letterOptionCount(difficulty, stufe) {
  if (stufe === "schwer") return 4;
  if (stufe === "mittel") return difficulty === "easy" ? 3 : 4;
  return difficulty === "easy" || difficulty === "medium" ? 3 : 4;
}
function letterModesFor(difficulty, stufe) {
  const table = LETTER_MODES[stufe] || LETTER_MODES.mittel;
  return table[difficulty] || table.easy;
}
...
function generateLetterTask(difficulty, stufe = currentStufe(), wantedMode = null) {
  ...
  if (mode === "wordStart" || mode === "wordEnd") {
    const letter = mode === "wordStart" ? word[0] : lastLetterOf(word);
    // Nur Wörter, die nicht auch mit diesem Buchstaben anfangen bzw. enden –
    // sonst wären zwei Antworten richtig.
    const others = shuffleOptions(LETTER_ITEMS.filter((other) => other !== item
      && (mode === "wordStart" ? other.word[0].toUpperCase() !== letter : lastLetterOf(other.word) !== letter)));
    const options = ensureUniqueTextOptions(item.word, others.slice(0, optionCount + 2).map((other) => other.word), optionCount);
    const question = mode === "wordStart" ? `Welches Wort beginnt mit ${letter}?` : `Welches Wort endet mit ${letter}?`;
    return {
      puzzleType: "letterPuzzle", difficulty, item, word, mode, letter, options,
  ...
  // Bei der Lücke steht das Wort nicht in der Frage – sonst stünde die
  // Antwort daneben. Der Lautsprecher darf es sagen: Hören und den Buchstaben
  // finden ist dann die Aufgabe.
  const question = mode === "gap" ? "Welcher Buchstabe fehlt?" : mode === "end" ? `Womit endet ${item.word}?` : `Womit beginnt ${item.word}?`;
  return {
    puzzleType: "letterPuzzle", difficulty, item, word, mode, gapIndex, options,
    correctAnswer: correct, questionText: question,
    speakText: mode === "gap" ? `Welcher Buchstabe fehlt im Wort ${item.word}?` : question,
  };
}
function letterTaskKey(task) { return ["letterPuzzle", task.word, task.mode, task.gapIndex ?? ""].join(":"); }
function makeLetterTaskView() {
  const task = state.task;
  const wordMode = task.mode === "wordStart" || task.mode === "wordEnd";
  const view = document.createElement("div");
  view.className = "practice-task letter-task";
  const emoji = document.createElement("div");
  emoji.className = `letter-image${wordMode ? " is-letter" : ""}`;
  emoji.setAttribute("aria-hidden", "true");
  // Zum Buchstaben das Wort suchen: dann steht der Buchstabe gross da, wo
  // sonst das Bild ist – das Bild verriete die Antwort.
  emoji.textContent = wordMode ? task.letter : task.item.emoji;
  const word = document.createElement("p");
  word.className = "letter-word";
  word.textContent = task.mode === "gap"
    ? [...task.item.word].map((letter, index) => (index === task.gapIndex ? "_" : letter)).join("")
    : task.item.word;
  if (wordMode) word.hidden = true;
  const question = document.createElement("p");
  question.className = "letter-question";
  question.textContent = task.questionText;
  view.append(emoji, word, question);
  return view;
}

// Was ein Level verlangt, in einem Satz – je Stufe, denn dieselbe Welt stellt
// auf "schwer" andere Aufgaben als auf "leicht" (READING_TASK_TYPES,
// LETTER_MODES). Der Katalog trägt den Satz der Mitte; gezeigt wird beim Start
// der zur Stufe (practiceDescription).
const READING_LEVEL_DESCRIPTIONS = {
  ...
  mittel: {
    easy: "Fehlende Buchstaben und das passende Wort zum Bild.",
    medium: "Wörter zum Bild und fehlende Silben.",
    hard: "Silben, Wörter und erste Sätze zum Bild.",
    extreme: "Sätze lesen und fehlende Wörter ergänzen.",
  },
  schwer: {
    easy: "Wörter zum Bild und fehlende Silben.",
    medium: "Silben ergänzen und den Satz zum Bild finden.",
    hard: "Sätze lesen und das fehlende Wort einsetzen.",
    extreme: "Sätze um das passende Wort ergänzen.",
  },
};
const LETTER_LEVEL_DESCRIPTIONS = {
  leicht: {
    easy: "Welcher Buchstabe kommt zuerst?",
    medium: "Anfangsbuchstaben üben.",
    hard: "Mehr Buchstaben zur Auswahl.",
    extreme: "Anfang und Ende der Wörter.",
  },
  mittel: {
    easy: "Welcher Buchstabe kommt zuerst?",
    medium: "Anfang und Ende der Wörter.",
    hard: "Anfang, Ende – und das Wort zum Buchstaben.",
    extreme: "Buchstaben mitten im Wort und Wörter zum Buchstaben.",
  },
  schwer: {
    easy: "Anfang und Ende der Wörter.",
    medium: "Das Ende und Buchstaben mitten im Wort.",
    hard: "Lücken im Wort und das Wort zum Buchstaben.",
    extreme: "Lücken im Wort, Wörter zu Anfang und Ende.",
  },
};
const PRACTICE_DESCRIPTIONS = { readingPuzzle: READING_LEVEL_DESCRIPTIONS, letterPuzzle: LETTER_LEVEL_DESCRIPTIONS };
function practiceDescription(level) {
  const table = PRACTICE_DESCRIPTIONS[level?.game];
  if (!table) return "";
  return (table[currentStufe()] || table.mittel)[level.difficulty] || "";
}
const PRACTICE_TARGET_BY_DIFFICULTY = { easy: 5, medium: 7, hard: 10, extreme: 10 };
function practiceTargetCount(difficulty) { return PRACTICE_TARGET_BY_DIFFICULTY[difficulty] || 10; }
function makePracticeLevels(game, descriptions) {
  return DIFFICULTY_KEYS.flatMap((difficulty) =>
    Array.from({ length: LEVELS_PER_DIFFICULTY }, (_, index) => {
      const targetCount = practiceTargetCount(difficulty);
      return makeLevel(game, difficulty, index + 1, {
        targetCount,
        badge: `${targetCount} Aufgaben`,
        description: descriptions.mittel[difficulty],
      });
    })
  );
}
const READING_LEVELS = makePracticeLevels("readingPuzzle", READING_LEVEL_DESCRIPTIONS);
const LETTER_LEVELS = makePracticeLevels("letterPuzzle", LETTER_LEVEL_DESCRIPTIONS);
...
function normalizeLevelCounts(levels) {
  // Der Garten zuerst, dann die vier Welten: so ist Garten 1 das erste Level
  // eines Spiels, und die Freischaltkette läuft von dort durch die Wiese.
  const starter = levels.filter((level) => level.difficulty === STARTER_KEY).slice(0, LEVELS_PER_STARTER);
  return [...starter, ...DIFFICULTY_KEYS.flatMap((difficulty) => levels
...
const LEVELS_BY_GAME = {
  ...
  letterPuzzle: normalizeLevelCounts(LETTER_LEVELS),
...
function levelStars(level) {
  const id = level.id || level.levelName;
  return kids()?.getStars?.(level.game, id) || 0;
}
function renderStarRow(count, options = {}) {
  const row = document.createElement("div");
  row.className = "star-row" + (options.small ? " star-row-small" : "");
  row.setAttribute("aria-label", `${count} von 3 Sternen`);
  for (let i = 0; i < 3; i += 1) {
    const star = document.createElement("span");
    star.className = "star" + (i < count ? " filled" : " empty");
    if (options.animate && i < count) { star.classList.add("to-reveal"); star.style.setProperty("--star-index", i); }
...
function isLevelUnlocked(level) {
  ...
  // Wiese 1 ist immer frei – und Garten 1, wo es den Garten gibt. Die Wiese
  // hängt nicht am Garten: Wer ihn nicht sieht, käme sonst nie hinein.
  const firstOfWorld = index === 0 || levels[index - 1].difficulty !== level.difficulty;
  if (firstOfWorld && (level.difficulty === "easy" || level.difficulty === STARTER_KEY)) return true;
...
function renderDifficultySelect() {
  ...
  const worlds = worldsFor(currentGame);
  levelGrid.className = `difficulty-grid${worlds.length > DIFFICULTY_KEYS.length ? " has-garten" : ""}`;
  levelGrid.setAttribute("aria-label", "Schwierigkeitsgrad auswählen");
  levelGrid.innerHTML = "";
  worlds.forEach((difficulty) => {
  ...
  setHelpText(`${sentence(config.title)} ${config.subtitle} Wähle zuerst deine Welt: ${worlds.map((world) => DIFFICULTIES[world].label).join(", ")}. Je weiter rechts, desto kniffliger.`);
}
...
function renderLevelSelect() {
  ...
  levelGrid.innerHTML = "";
  levelsForDifficulty(selectedDifficulty).forEach((level, position) => {
...
function revealSuccessContent() {
  ...
  if (successOverlay && !prefersReducedMotion()) {
...
function showSuccess() {
  ...
  // Auf der Stufe "schwer" verlangt der Auftrag drei Sterne (needStars,
  ...
  if (journeyTask && isJourneyLevel(level)) {
    ...
    if (stars >= need) {
      ...
      journeyStamp = { missing: true, need };
    }
  }
...
function updateSuccessContent() {
  ...
  if (kids()?.mascotSVG) {
    ...
    mascot.className = "success-summary success-mascot";
    mascot.setAttribute("aria-hidden", "true");
    mascot.innerHTML = kids().mascotSVG("cheer");
    successContent.append(mascot);
  }

  const starWrap = document.createElement("div");
  starWrap.className = "success-summary success-stars";
  starWrap.append(renderStarRow(celebration.stars || 1, { animate: true }));
  successContent.append(starWrap);
  ...
  if (celebration.journeyStamp) {
    ...
    stamp.textContent = celebration.journeyStamp.missing
      ? `Für den Stempel brauchst du ${celebration.journeyStamp.need === 3 ? "drei" : "zwei"} Sterne – spiel das Level noch einmal!`
      : celebration.journeyStamp.gold ? "Auftrag geschafft – ein goldener Stempel!" : "Auftrag geschafft – Stempel für die Karte!";
  ...
  overlay.innerHTML = `
    <div class="success-modal">
      ...
      <div class="success-actions" aria-label="Level-Aktionen">
        <button id="success-restart-button" class="success-text-button" type="button" aria-label="Level neu starten">🔁 Nochmal</button>
        <button id="next-puzzle-button" class="success-text-button primary" type="button" aria-label="Nächstes Level">Weiter →</button>
      </div>
    </div>`;
...
// Anzahl der Antwortkarten, die nach mehreren Fehlversuchen ausgegraut werden.
function hintDimCount() {
...
function renderPracticeOptions(onAnswer) {
  ...
  const revealCorrect = state.awaitingNext || state.completed;
  const attempts = state.attempts || 0;
  const dimTarget = hintDimCount();
  let dimmed = 0;
  state.task.options.forEach((option) => {
    const isCorrect = sameAnswer(option, state.task.correctAnswer);
    const button = document.createElement("button");
    button.type = "button";
    let className = `practice-option${optionStateClass(option)}`;
    let disabled = state.awaitingNext || state.completed;
    if (!revealCorrect) {
      if (!isCorrect && dimmed < dimTarget) { className += " dimmed"; disabled = true; dimmed += 1; }
      else if (isCorrect && attempts >= 3) className += " hint";
    }
...
function makeReadingDisplay(task) {
  ...
  display.className = task.taskType === "sentenceMissingWord" || task.taskType === "sentenceComplete" ? "reading-display sentence-gap" : "letter-row";
...
const GAME_HANDLERS = {
  ...
  letterPuzzle: {
    resetState(level) { resetPracticeState(level, generateLetterTask, "Welcher Buchstabe passt zum Wort?"); },
    checkWin() { return Boolean(state.completed); },
    answer(answer) {
    ...
    render(level) {
      renderPracticeShell(level, "letterPuzzle-board", makeLetterTaskView(), (answer) => this.answer(answer), generateLetterTask, "Neues Wort.");
    },
  },
...
// Kommt die Stufe aus der Cloud erst nach dem ersten Bild an (oder stellen
